...
| **Outbound** | 10[.]0[.]2[.]xxx:5044 | ALLOW | Logstash 전송 |
...
| **Inbound** | 5044/tcp | ALLOW (10[.]0[.]1[.]xxx만) | Logstash |
...
| `a5c41ad2a873e790...` | (직접 업로드) | 2건 | 2026-02-27 | Azure IP(20[.]61[.]127[.]xxx)에서 업로드 |
...
3. **프록시/감염 호스트 이용**: 접속 IP(`src_ip`)와 다운로드 URL의 IP가 불일치하는 케이스 다수 발견
   - 예: `src_ip: 78[.]70[.]152[.]xxx` → `download: hxxp://175[.]214[.]227[.]xxx:50962/.i`
...
`45[.]153[.]34[.]xxx`에서 다운로드된 `cat.sh` 셸 스크립트(SHA256: `c7999c2a...`)는 동일 IP에서 3회 반복 다운로드가 시도되었다. 이는 초기 침투 시 멀티 아키텍처 바이너리를 순차적으로 다운로드·실행하는 전형적인 **봇넷 드로퍼 패턴**이다.
